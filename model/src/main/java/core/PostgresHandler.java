...
public class PostgresHandler {
    ...
    private static final Logger logger = Logger.getLogger(PostgresHandler.class);
	...
	private void createConnection() throws Exception {
		...
		this.conn = DriverManager.getConnection(this.dburl, this.userName,
				this.password);
	}
	 ...
	 */
	public int[][] getIntArrayFromRows(String schemaName, String tableName,
			String queryStr, String queryFieldName, int[] colIdx) {
		if (!this.getTypeOfField(schemaName, tableName, queryFieldName).equals(
				String.valueOf(SynPopGenerator.DBVariableType.text))){
			return null;
		}
		
		for (int icol = 0; icol <= colIdx.length - 1; icol++){
			if (!this.getTypeOfField(schemaName, tableName, colIdx[icol])
					.equals(String.valueOf(SynPopGenerator.DBVariableType.int4))){
				return null;
			}
		}
		...
		try {
			...
			while (rs.next()){
				if (rs.getString(queryFieldName).equals(queryStr)) {
					...
					intArr = (new ArrayHandler()).concateMatrices(intArr,
							tmpIntArr);
				}
			}
