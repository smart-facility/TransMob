...
public class ArrayHandler {
	 ...
	 */
	public int[] getNonCommonInArrays(int[] array1, int[] array2) {
		int[] nonCommon = null;
	 ...
	 */
	public int[] getUnique(int[] srcArray) {

        List<Integer> uArray = new ArrayList<>();
	 ...
	 */
	public int roundToCloserInt(double val) {
		if (val > 0) {
			return (int) (val + 0.5);
        } else {
			return (int) (val - 0.5);
	 ...
	 * @return
	 */
	public int[] makeIncrementalIntArray(int startVal, int endVal, int step) {
	 ...
	 * @return
	 */
	public int[] makeUniformArray(int size, int val) {
	 ...
	 * @return
	 */
	public int[] extractArray(int[] srcArray, int iStart, int iEnd) {
	 ...
	 * @return
	 */
	public int[] extractArray(int[] srcArray, int[] idxArray) {
		if (this.max(idxArray) > srcArray.length - 1) {
			return null;
        }
	 ...
	 * @return
	 */
	public int[] getColumn(int[][] srcArray, int colIdx) {
	 ...
	 * @return
	 */
	public int[][] getColumns(int[][] srcArray, int[] colIdx) {
		if (colIdx[this.getIndexOfMax(colIdx)] > srcArray[0].length) {
			return null;
        }
	 ...
	 * @return
	 */
	public int[][] replaceColumn(int[][] srcArray, int colIdx, int[] newCol) {
		if (newCol == null) {
			return srcArray;
        }
	 ...
	 * @return
	 */
	public int[][] concateColumns(int[] col0, int[] col1) {
	 ...
	 * @return
	 */
	public int[][] concateColumns(int[][] srcArr, int[] col) {
	 ...
	 * @return
	 */
	public int[] subtractValuesInColumns(int[] col0, int[] col1) {
		if (col0.length != col1.length) {
			return null;
        }
	 ...
	 * @return
	 */
	public int[] addValuesInColumns(int[] col0, int[] col1) {
		if (col0.length != col1.length) {
			return null;
        }
		int[] finArr = new int[col0.length];
	 ...
	 * @return
	 */
	public int[][] addPositiveIn2DArrays(int[][] arr1, int[][] arr2) {
	 ...
	 * @return
	 */
	public int sumPositiveInArray(int[] srcArray) {
		int sum = 0;
		for (int i = 0; i <= srcArray.length - 1; i++) {
			if (srcArray[i] > 0) {
				sum = sum + srcArray[i];
	 ...
	 */
	public int[] pickRandomFromArray(int[] srcArray, int[] excludedArray,
		...
		int[] remained = (new ArrayHandler()).getNonCommonInArrays(srcArray,
				excluded);
		if (remained.length < nPick) {
			...
			for (int i = 0; i <= nPick - 1; i++) {
				...
				remained = this.getNonCommonInArrays(srcArray, excluded);
			}
		}
		return finArr;
	}
	 ...
	 *         specified value)
	 */
	public int[] removeValueInArray(int[] tmparray, int remVal) {
		int[] finarray = null;
		if (tmparray == null) {
			return finarray;
        }
		...
		finarray = this.extractArray(tmparray, idxRemain);
	 ...
	 *         value.
	 */
	public int[] getIndexofClosestToValue(int[][] tmparray, int val,
			boolean symmetric) {
		if (tmparray == null)
			return null;
	 ...
	 */
	public int getIdxOfNextGreaterValue(int[] srcArray, int minVal) {
		if (this.max(srcArray) - minVal < 0) {
			return -1;
        }
		int iMinPosDif = this.getIndexOfMax(srcArray);
		for (int i = 0; i <= srcArray.length - 1; i++) {
	...
	public int[][] concateMatrices(int[][] tmp1, int[][] tmp2) {
	 ...
	 */
	public int[] sortedIndices(int[] x) {
		int[] ix = new int[x.length];
		int[] scratch = new int[x.length];
		for (int i = 0; i < ix.length; i++) {
			ix[i] = i;
	 ...
	 */
	public int[] allocateProportionally(int[] srcArr, int sum) {
		...
		for (int i = 0; i <= finArr.length - 1; i++) {
			if (srcArr[i] <= 0) {
				...
				finArr[i] = this.roundToCloserInt((double) srcArr[i]
						/ (double) this.sumPositiveInArray(srcArr) * sum);
            }
        }

		int tstSum = this.sumPositiveInArray(finArr);

		if (tstSum > sum) {
			int[] indices = this.sortedIndices(finArr);
		...
		if (tstSum < sum) {
			int[] indices = (new ArrayHandler()).sortedIndices(finArr);
	 ...
	 * @return an integer array satisfying the above conditions.
	 */
	public int[] allocateProportionally(double[] srcArr, int sum) {
		int[] finArr = new int[srcArr.length];
		...
		for (int i = 0; i <= finArr.length - 1; i++) {
			if (srcArr[i] < 0) {
				...
				finArr[i] = this.roundToCloserInt(srcArr[i]*sum);
            }
        }
	 ...
	 */
	public int max(int[] srcArray) {
		int imax = this.getIndexOfMax(srcArray);
	 ...
	 */
	public int getIndexOfMax(int[] srcArray) {
		int maxIdx = 0;
		for (int i = 0; i <= srcArray.length - 1; i++) {
	 ...
	 */
	public int min(int[] srcArray) {
		int imin = this.getIndexOfMin(srcArray);
	 ...
	 */
	public int getIndexOfMin(int[] srcArray) {
		int minIdx = 0;
		for (int i = 0; i <= srcArray.length - 1; i++) {
